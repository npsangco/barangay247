                    ...
                    <form action="{{ route('incidents.store') }}" method="POST">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="resident_id" name="resident_id" required>
                                    ...
                                    @foreach($residents as $resident)
                                        <option value="{{ $resident->resident_id }}" {{ old('resident_id') == $resident->resident_id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="incident_type" name="incident_type" value="{{ old('incident_type') }}" required>
                            </div>
                            <div class="col-md-6">
                                <label for="date_reported" class="form-label">Date Reported <span class="text-danger">*</span></label>
                                <input type="date" class="form-control" id="date_reported" name="date_reported" value="{{ old('date_reported') }}" required>
                            </div>
                            <div class="col-md-12">
                                <label for="incident_details" class="form-label">Incident Details <span class="text-danger">*</span></label>
                                <textarea class="form-control" id="incident_details" name="incident_details" rows="4" required>{{ old('incident_details') }}</textarea>
                            </div>
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-check-lg me-1"></i> Create Incident
                                </button>
                                <a href="{{ route('incidents.index') }}" class="btn btn-secondary">
                                    Cancel
                                </a>
